// Write menu driven program to implement operations on array.   1)Add  2) Delete 3) Modify 4) sort 5) simple search 6) binary search

#include <stdio.h>

//Add Element 
int addElementToArray(int arr[], int element, int position) {
  if (position == 4) {
    printf("Array is Full. No space available to add.");
  } else {
    position++;
...
//Delete Element
int deleteElementFromArray(int arr[], int position,int element){
...
//Modify Element
void modifyElementToArray(int arr[],int position,int element,int newElement){
  int temp=0,i;
  if(position==-1){
    printf("Array is Empty");
  }else{
...
//Sort Array
void sortToArray(int arr[],int position){
  int i,j,temp;
  if(position==-1){
    printf("Array is Empty");
  }else{
...
void main() {
  ...
  while (1) {
    ...
    case 1:
      ...
      scanf("%d", &element);
      position = addElementToArray(arr, element, position);
      break;

    case 2:
      printf("Enter the element to delete : ");
      scanf("%d",&element);
      position = deleteElementFromArray(arr, position, element);
      break;
    ...
    case 3:
      ...
      scanf("%d",&newElement);
      modifyElementToArray(arr,position,element,newElement);
      break;
